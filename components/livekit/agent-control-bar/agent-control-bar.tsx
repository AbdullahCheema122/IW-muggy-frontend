...
export interface AgentControlBarProps
  ...
  /** Optional: still called after navigation so you can stop timers, etc. */
  onEndCall?: () => void;
...
export function AgentControlBar({
  ...
  // 1) await internal disconnect (mic/cam off + room disconnect)
  // 2) navigate to main page "/"
  // 3) fire onEndCall/onDisconnect so parent can do any extra cleanup
  const onLeave = async () => {
    ...
    try {
      ...
      console.warn("handleDisconnect failed; proceeding to navigate home", e);
    } finally {
      setIsDisconnecting(false);
      // Navigate to main page
      router.push("/");
      // Let parent do extra cleanup if desired
      ...
      <div className="flex flex-row justify-between gap-1">
        ...
        {visibleControls.leave && (
          ...
          >
            ...
            <span className="hidden md:inline">{isDisconnecting ? "ENDING…" : "END CALL"}</span>
            <span className="inline md:hidden">{isDisconnecting ? "…" : "END"}</span>
          </Button>
        )}
      </div>
    </div>
